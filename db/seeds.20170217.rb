...
BiomutaUrl = "https://hive.biochemistry.gwu.edu/tst/biomuta/content/BioMuta3.csv"
...
def getProteinId(uniprot)
	begin
		...
		puts "Error en getProteinId, reintentando..."
		retry
	end
	valueRet = ""
...
def downloadNoZipURL(url)
		...
		puts "Error en downloadNoZipURL, reintentando..."
                retry
        rescue
                puts "Error: #{$!}"
        end
...
def processBiomuta
	system("wget "+BiomutaUrl+" -O /home/joan/apps/bionotes/data/download/BioMuta.csv")
	data = `awk -F"\t"  '{if($10!=$11 && $12!="-" && $13!="-")print $0}' /home/joan/apps/bionotes/data/download/BioMuta.csv | sort -k2`
	data = data.split(/\n/)
	puts "Biomuta data download OK"
        mutations = { 'XXX' => {} }
	puts "Biomuta processing data"
	__uniprotAc = 'XXX'
	n = data.length.to_i
        nn = data.length.to_i
        puts("processing "+nn.to_s+" lines")
        data.each do |l|
                tmp = {}
                linea = l.chomp.split("\t")
		...
		if uniprotAc != __uniprotAc
			mutations[ __uniprotAc ].each do |k,v|
				Biomutaentry.create(proteinId:__uniprotAc,resId:k,data:v.to_json)
			end
			mutations = {}
			mutations[ uniprotAc ] = {}
...
puts "Seeds.rb started"

#puts "Updating  Biomuta"
#processBiomuta
#puts "Biomuta updated"
