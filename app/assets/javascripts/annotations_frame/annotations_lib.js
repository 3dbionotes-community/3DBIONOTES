...
function update_asa_residues(n){
  ...
  top.asa_residues[ n_model-1 ][ chain ].forEach(function(i){
    ...
    if(b<0)b = 0;
    var color = 'rgb('+r+',0,'+b+')';
    ASA[ parseInt(i[0]) ].variants = [{ color:color, alternativeSequence:'', type:'measure', begin: i[0], end: i[0], score:i[1], internalId:'asa_'+n, description:'<b style=\"color:grey;\">Accessible surface area</b><br/>Residue accesibility '+i[1]+parseFloat(i[1]).toFixed(2)+'&#8491;<sup>2</sup>' }];;
    n++;
  });
}
