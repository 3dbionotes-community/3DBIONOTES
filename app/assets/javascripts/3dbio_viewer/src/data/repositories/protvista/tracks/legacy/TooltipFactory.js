...
var createTooltipBox = function (fv, container) {
    ...
    var tooltipContainer = container.append("div").attr("class", "up_pftv_tooltip-container");
    tooltipContainer
        .append("span")
        .text("x")
        .attr("class", "up_pftv_tooltip-close")
        .on("click", function () {
            tooltipContainer.transition(20).style("opacity", 0).style("display", "none");
            ...
            if (association.description) {
                ...
                if (index !== -1) {
                    ...
                    association.description = (
                        association.description.slice(0, index) +
                        association.description.slice(index + 16)
                    ).trim();
                }
            }
        });
...
var Tooltip = function (fv, catTitle, d, container, coordinates) {
    ...
    if (keys || tooltip.data.sourceType !== undefined) {
        ...
        if (tooltip.data.sourceType === Evidence.variantSourceType.mixed) {
            sourceText = keys
                ? "UniProt, large scale studies and custom data (" + keys + ")"
            ...
            isLSS = true;
            sourceText = keys
                ? "Large scale studies and custom data (" + keys + ")"
                : "Large scale studies";
        } else {
...
var addEvidenceXRefLinks = function (tooltip, sourceRow, info) {
    if (!sourceRow) {
        sourceRow = tooltip.table.append("tr").attr("class", "up_pftv_evidence-source");

        sourceRow.append("td").text("");
    }
...
Tooltip.prototype.addEvidences = function (evidences) {
    ...
    _.each(evidences, function (sources, eco) {
        ...
        typeRow.append("td").text("Evidence");
        var evidenceText = getEvidenceText(tooltip, eco, sources);
        typeRow.append("td").text(evidenceText);
...
var AntigenTooltipViewer = function (tooltip) {
    if (tooltip.data.matchScore) {
        ...
        seqRow.append("td").text(function () {
...
var addPredictions = function (tooltip, data) {
    ...
    if (
        data.polyphenPrediction &&
        data.polyphenPrediction !== "-" &&
        data.polyphenInUse !== false
    ) {
...
var addAssociation = function (tooltip) {
    if (Evidence.existAssociation(tooltip.data.association)) {
        ...
        assocRow
            .append("td")
            .attr("colspan", 2)
            .classed("up_pftv_subsection", true)
...
var addXRefs = function (tooltip, xrefs) {
    if (xrefs && xrefs.length !== 0) {
        var sourceRow = tooltip.table.append("tr").attr("class", "up_pftv_evidence-source");

        sourceRow.append("td").text("Cross-references");
...
var addUPSection = function (tooltip, upEvidences, upXrefs) {
    if (
        tooltip.data.ftId ||
        tooltip.data.up_description ||
        upEvidences.length !== 0 ||
        tooltip.data.association
    ) {
