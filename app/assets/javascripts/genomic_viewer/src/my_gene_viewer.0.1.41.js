  ...
  d3.svg.axis = function() {
    ...
    function axis(g) {
      g.each(function() {
        ...
        text[0].forEach(function(i){
                i.innerHTML = parseInt( i.innerHTML )+parseInt( g.attr('index_shift') );
        });
...
//        var nxSeq = sequence.startsWith('NX_') ? true : false;
        ...
        d3.helper.tooltip = function (object) {
			...
			var first_line = '<p style="margin:2px;color:orangered">' + (parseInt(pD.x)+parseInt(self['index_shift'])) + '</p>';
			...
			var first_line = '<p style="margin:2px;color:orangered">' + (parseInt(pD.x)+parseInt(self['index_shift'])) + ' - ' + (parseInt(pD.y)+parseInt(self['index_shift'])) + '</p>';
                        if (pD.description) var second_line = '<p style="margin:2px;color:white;font-size:9px">' + pD.description + '</p>';
                        else var second_line = '';
                    }
...
//                            $('#tLineC').text(elemHover.y);  
                        ...
                        if (rightside) {
                            ...
                            tooltipDiv.style({
                                left: (absoluteMousePos[0] + 10 - (tooltipDiv.node().getBoundingClientRect().width)) + 'px',
                                top: (absoluteMousePos[1] - 100) + 'px'//(absoluteMousePos[1] - 55) + 'px'
                            });
                        } else {
                    ...
                    .on('click', function (pD, pI) {
                        ...
                        if (object.type === "path") {
                            ...
                            xTemp = pD.x - 0.4;
                            yTemp = pD.y + 0.4;
                        } else if (  object.type === 'rect' && pD.x === pD.y-1 ) {
                            xTemp = pD.x - 0.4;
                            yTemp = pD.y + 0.4;
                        }else {
                            xTemp = pD.x;
			...
			if(widthRect<1 && widthRect>=0)widthRect=1;
                        selectedRect.style({
                            left: xRect + 'px',
                            top: ($(div + " .svgHeader").length) ? 60 + 'px' : 10 + 'px',
                            'background-color': 'rgba(255, 233, 153, 1)',//'background-color': 'rgba(0, 0, 0, 0.2)',
                            width: widthRect + 'px',
        ...
        function rectWidth2(d){
            ...
            if (d.x === d.y-1) {
                return (scaling(d.y+0.4) - scaling(d.x-0.4));
            }
            return (scaling(d.y) - scaling(d.x));
        };
...
//          .interpolate("cardinal")
		...
		.attr("index_shift",self['index_shift'])
                .call(xAxis);
        }
        ...
        var fillSVG = {
            typeIdentifier: function (object) {
                if (object.type === "rect") {
                    ...
                    if (displaySequence(object.data.length)) {
                        fillSVG.sequence(object.data, Yposition);
                    }
                    yData.push({
                      title: object.name,
            ...
            sequence: function (seq, position, start, className) {//sequence: function (seq, position, start) {
                //Create group of sequence
                if (!start) var start = 0;
		if(!className)className="AA";
                svgContainer.append("g")
                    .attr("class", "seqGroup")
                    .selectAll("."+className)//.selectAll(".AA")
                    .data(seq)
                    .enter()
                    .append("text")
                    .attr("clip-path", "url(#clip)")
                    .attr("class",className)//.attr("class", "AA")
...
//                scaling.range([5,width-5]); 
                if (SVGOptions.showSequence && !(intLength) && seq && svgContainer.selectAll(".AA").empty()) {
                    ...
                    features.forEach(function(i){
                        if(i.id=="protein_seq"){
                                fillSVG.sequence(i.data.substring(start-1, end), 60, seqShift-1,"pep");
                        }
                        if(i.id=="neg_seq"){
                                fillSVG.sequence(i.data.substring(start-1, end), 40, seqShift-1,"neg");
                        }
                        if(i.id=="uniprot_seq"){
                                fillSVG.sequence(i.data.substring(start-1, end), 60, seqShift-1,"uni");
                        }
                    });
                }
...
//            console.log(current_extend);
        ...
        this.__highlight = function (__x,__y) {
            ...
            selectedRect.style({
                left: xRect + 'px',
                top: 60 + 'px',
                'background-color': 'rgba(255, 233, 153, 1)',
                width: widthRect + 'px',
                height: (Yposition + 50) + 'px',
                position: 'absolute',
                'z-index': -1,
        ...
        function initSVG(div, options) {
            ...
            if (options.toolbar === true) {
                ...
                if (!$(div + ' .header-position').length) {
                    var headerPosition = headerOptions
                        ...
                        .style("display", "inline-block")
                        .style("width", "250px")//.style("width", "175px")
            ...
            svgContainer.on('mousemove', function () {
                ...
                var __shift =  parseInt( $(div + " #zoomPosition").attr('index_shift') );
		var __neg_dna = {'A':'T','T':'A','C':'G','G':'C'};
                $(div + " #zoomPosition").text( Math.round(scalingPosition(absoluteMousePos[0]))+__shift+" - "+sequence[pos-1]+" : "+__neg_dna[ sequence[pos-1] ]);
            });
            ...
            if (options.showSequence && !(intLength)) {
                ...
                yData.push({
                    title: "POS STRAND",//title: "Sequence",
                    y: Yposition - 8
                });
            }
