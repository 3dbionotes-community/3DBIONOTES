...
//        var nxSeq = sequence.startsWith('NX_') ? true : false;
	...
	if(options['index_shift'] > 0){
        ...
        var fillSVG = {
            ...
            sequence: function (seq, position, start, className) {
...
//                    .style("shape-rendering", "crispEdges")
	    ...
	    //console.log( extent );
			...
			if(i.id=="protein_seq"){
				fillSVG.sequence(i.data.substring(start, end), 60, seqShift,"pep");
			}
			if(i.id=="neg_seq"){
				fillSVG.sequence(i.data.substring(start, end), 40, seqShift,"neg");
			}
			if(i.id=="uniprot_seq"){
				fillSVG.sequence(i.data.substring(start, end), 60, seqShift,"uni");
			}
	    	    });
                }
	...
	self.__clear = function() {
        ...
        self.__zoom_in = function() {
	    var tmp_highlighted = null;
	    if(this.highlighted){
		tmp_highlighted = this.highlighted;
	    }
	    if(tmp_highlighted){
               var __x = tmp_highlighted[0]-2;
               var __y = __x+51;
               self.__zoom_to(__x,__y);
	    }else{
		self.__zoom_to(1,52);
	    }
        }
	self.__zoom_to = function(__x,__y) {
	    var tmp_highlighted = null;
	    if(this.highlighted){
		tmp_highlighted = this.highlighted;
	    }
	    ...
	    if(tmp_highlighted) this.__highlight( tmp_highlighted[0] , tmp_highlighted[1] );
        }
		...
		var __shift =  parseInt( $(div + " #zoomPosition").attr('index_shift') );
                $(div + " #zoomPosition").text( Math.round(scalingPosition(absoluteMousePos[0]))+__shift);
            });
