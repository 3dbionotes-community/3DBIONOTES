module AlignmentsEnsemblManager
  module BuildAlignments
    ...
    include ToolsEnsembl::BuildEnsemblAlignment
    def buildENSEMBLdataTranscriptViewer(ensembl_gene_id,ensembl_transcript_id,uniprot_acc=nil)
      ensembl_obj = EnsemblDatum.find_by(geneId: ensembl_gene_id, transcriptId: ensembl_transcript_id, proteinId: uniprot_acc)
      if ensembl_obj.nil?
        ...
        EnsemblDatum.create(geneId: ensembl_gene_id, transcriptId: ensembl_transcript_id, proteinId: uniprot_acc, data: {'transcript'=>transcript,'gene'=>gene['gene'],'aa_seq'=>transcript_peptide_seq}.to_json)
        return {'transcript'=>transcript,'gene'=>gene['gene'],'aa_seq'=>transcript_peptide_seq}
      else
        return JSON.parse(ensembl_obj.data)
      end
    end
